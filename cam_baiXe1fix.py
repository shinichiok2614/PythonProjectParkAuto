...
class App:
    ...
    def toggle_pause(self):
        ...
        if self.paused:
            ...
            # ⭐ THÊM DÒNG NÀY
            self.handle_multi_plate_on_pause()

        else:
            self.btn_save.pack_forget()

    def handle_multi_plate_on_pause(self):
        """
        Khi pause:
        - tìm car_id có nhiều plate
        - mở cửa sổ xác nhận thủ công
        """
        from manual_plate_assign import resolve_multi_plate

        # group entries theo car_id
        grouped = {}
        for e in self.latest_entries:
            grouped.setdefault(e["car_id"], []).append(e)

        new_entries = []
        next_car_id = max([e["car_id"] for e in self.latest_entries]) + 1 if self.latest_entries else 1000

        for car_id, items in grouped.items():
            if len(items) <= 1:
                new_entries.extend(items)
                continue

            # ⭐ car_id có nhiều plate → xử lý thủ công
            resolved, next_car_id = resolve_multi_plate(
                parent=self.root,
                car_entries=items,
                start_car_id=next_car_id
            )

            new_entries.extend(resolved)
    ...
    def video_loop(self):
        ...
        try:
            while self.running:
                ...
                self.last_frame_entries = frame_entries
